...
{
	this->chassis.Sync();
	for (int8_t i = this->_device_count-1; i >= 0; --i) this->_devices[i]->Sync();
}
...
{
	if ((DeviceIndex + 1) > this->_device_count) return -2;
	for (uint8_t d = DeviceIndex; (d + 1) < this->_device_count; d++) this->_devices[d] = this->_devices[d + 1];
