...
void Chassis::_init(uint8_t *PWM_pins, uint8_t *DIR_pins, uint8_t *ENCODER_pins)
{
	...
	for (int8_t w = WHEEL_COUNT - 1; w >= 0; --w)
	{
		...
		attachInterrupt(digitalPinToInterrupt(this->_wheel_ENCODER_pins[w]), wheelISR[w], RISING);
...
void Chassis::sync()
{
	...
	if (this->_wheel_sync_move)
	{
		...
		for (w = WHEEL_COUNT - 1; w >= 0; --w)
			analogWrite(_wheel_PWM_pins[w], U0[w]);
	}
...
void Chassis::wheelCalibrate(int8_t wheel)
{
	...
	while ((this->_wheelAngSpeed[wheel] == 0) && (U <= 255))
	{
		...
		for (int8_t i = MEAN_DEPTH - 1; i >= 0; --i)
			wheelTime = wheelTime + _IRQ_wheelTimeArray[wheel][i];
		if (wheelTime > 0)
		{
	...
	for (int8_t i = 0; i < WHEEL_TABLE_SIZE; i++)
	{
		...
		for (int8_t m = MEAN_DEPTH - 1; m >= 0; --m)
			wheelTime = wheelTime + _IRQ_wheelTimeArray[wheel][m];
...
int Chassis::_eepromWriteWheelTable(int8_t wheel, int *table)
{
	...
	for (unsigned int i = 0; i < N; i++)
		EEPROM.update(addr + i, *((int8_t *)table + i));
...
int Chassis::_eepromReadWheelTable(int8_t wheel, int *table)
{
	...
	for (unsigned int i = 0; i < N; i++)
		*((int8_t *)table + i) = EEPROM.read(addr + i);
...
int Chassis::_wheelGetU(float ang_speed, int wheel, float volts)
{
	...
	if ((ang_speed < (float)this->getWheelTableValue(wheel, SPEED, 0)) || (ang_speed > (float)getWheelTableValue(wheel, SPEED, WHEEL_TABLE_SIZE - 1)))
		return -1;

	for (i = WHEEL_TABLE_SIZE - 1; i >= 0; --i)
	{
		if ((float)this->getWheelTableValue(wheel, SPEED, i) < ang_speed)
			break;
	}
...
int Chassis::_wheelGetBDelay(float ang_speed, int wheel)
{
	...
	if (fabs(ang_speed) < (float)this->getWheelTableValue(wheel, SPEED, 0))
		return 0;
	if (fabs(ang_speed) > (float)this->getWheelTableValue(wheel, SPEED, WHEEL_TABLE_SIZE - 1))
		return (float)this->getWheelTableValue(wheel, SPEED, WHEEL_TABLE_SIZE - 1);
...
int Chassis::wheelRotateAng(float *speed, float *ang, bool en_break)
{
	...
	for (w = WHEEL_COUNT - 1; w >= 0; --w)
	{
		if ((fabs(speed[w]) < (float)this->getWheelTableValue(w, SPEED, 0)) && (fabs(speed[w]) > 0))
	...
	{
		this->_wheelDir[w] = 1;
		if ((this->_wheelSetAng[w] * this->_wheelSetAngSpeed[w]) < 0)
			this->_wheelDir[w] = -1;
...
void Chassis::_wheel_rotate_sync(bool en_break)
{
	...
	while (1)
	{
		...
		if (complete)
			break; //Exit from "while" if rotate finished
		...
		for (w = WHEEL_COUNT - 1; w >= 0; --w)
		{
			if (_IRQ_syncloop[w])
			{
				...
				if ((w == 0) && (pid_i < DEBUG_WHEEL_PID_I_DEPTH) && (k > 0))
				{
					...
					if (fabs(Error[0] / this->_wheelSetAngSpeed[0]) > debug_wheel_pid_max_error)
						debug_wheel_pid_max_error = fabs(Error[0] / this->_wheelSetAngSpeed[0]);
					pid_i++;
				}
#endif
			}
